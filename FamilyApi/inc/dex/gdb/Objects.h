...
#ifndef _DEX_OBJECTS_H_
#define _DEX_OBJECTS_H_
 ...
 *
 * It stores a collection of Dex object identifiers as a set. As a set,
 ...
 *
 * Object identifiers cannot be removed from an Objects instance while 
 * iterators are still traversing the collection.
 *
 * Also, in case Objects has been retrieved from the Graph, this
 * collection cannot be updated through its Graph. For example,
 * if we retrieve all the objects belonging to a given node or edge type, we
 * cannot add or drop nodes or edges of this type while traversing the 
 * retrieved collection.
 *
 * \author Sparsity Technologies http://www.sparsity-technologies.com
 */
class DEX_EXPORT Objects
: private Handler<dexcore::Objects>
{
...
public:
     ...
     */
    oid_t Any() throw(dex::gdb::NoSuchElementException, dex::gdb::Error);
...
private:
     ...
     * \param core_objs [in] Handled dexcore Objects reference.
     * \param s [in] Parent Session reference.
     */
    Objects(dexcore::Objects * core_objs, Session * s);
